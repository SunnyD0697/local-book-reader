...
2. 下载 `local-book-reader-0.3.0.zip`。**不要**下载 GitHub 自动生成的 **Source code (zip)**；它是源代码，不是安装包。
3. 关闭 Obsidian，或在替换文件前先停用 Local Book Reader。
4. 创建文件夹：`<Vault>/.obsidian/plugins/local-book-reader/`。
5. 将专用插件 ZIP 的内容解压到该文件夹中。确认以下四个运行文件直接位于 `local-book-reader` 文件夹内：
   - `main.js`
   - `manifest.json`
   - `styles.css`
   - `pdf.worker.min.mjs`
6. 重新打开 Obsidian，在“社区插件”中启用 **Local Book Reader**，然后使用 **从个人书库打开电子书** 或 **打开个人图书馆**。

ZIP 中还可能包含 `LICENSE`、`THIRD_PARTY_NOTICES.md` 和 `LICENSES` 文件夹。它们是许可证文件，不影响插件运行。

Local Book Reader 尚未进入 Obsidian 社区插件目录。标准的社区插件安装器目前不会获取本版本所需的额外 PDF Worker 文件。

## 从源代码构建并安装

1. 安装 Node.js `22.13.0` 或更新版本，以及 pnpm `11.9.0`。
2. 在仓库目录中运行 `pnpm install --frozen-lockfile` 和 `pnpm run build`。
3. 按上文第 3–6 步操作，但改为复制构建得到的四个运行文件，而不是解压发布 ZIP。
